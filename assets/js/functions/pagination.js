...
const Pagination = (() => {
  const _obj = {
    ...
    loadLinks: (item) => {
      _obj.el = Dom.el(`[data-pagination="${item}"]`);
      if (!_obj.el) {
        return;
      }
      ...
      Dom.els('[data-pg-count]', _obj.el).forEach(countElement => {
      ...
      Dom.els('[data-pg-link]', _obj.el).forEach(linkElement => {
      ...
      Dom.els('[data-pg-handler]', _obj.el).forEach(handlerElement => {
        Dom.setEventHandler(
    ...
    // Populates 'Per page' <select> options
    ...
    html: () => {
      let el = `<div class="pagination-meta">
                  <div class="pagination-limit">
                    <label for="pg-per-page" class="font-weight-bold">Per page: </label>
                    <!-- On <select> change, call event handler Pagination:pageLimitChanged() -->
                    <select id="pg-per-page" data-pg-handler="change:pageLimitChanged">
                      {pgOptions}
                    </select>
                  </div>
                  <div class="pagination-summary">
                    ...
                    to <span class="pg-count" data-pg-count="to">1</span>
